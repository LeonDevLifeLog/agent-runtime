...
> 二进制类工具（gh / yq / glab / multica / chsrc）版本在 `Dockerfile` 顶部 `ARG` 锚定；apt 装的随 Ubuntu 源浮动。
> gh 特意用二进制而非官方 apt 源，避免镜像内残留 `cli.github.com` 的 apt 源在境内运行时导致 `apt update` 卡死。
